    ...
    def spawn_job(self, call_id: int, fn: Callable[[], Any]) -> None:
        ...
        def _run() -> None:
            try:
                ...
                if exc.__class__.__name__ == "StopRequested" or str(exc) == "Stopped":
                    self.emit({"type": "stopped", "id": int(call_id)})
                else:
                    self.emit(
                        {
                            "type": "error",
                            "id": int(call_id),
                            "error": {"type": exc.__class__.__name__, "message": str(exc), "traceback": tb.format_exc()},
                        }
                    )
            finally:
...
def run_worker(dispatch: Callable[[WorkerProtocol, int, str, dict[str, Any]], None]) -> int:
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            proto.emit({"type": "error", "id": call_id, "error": {"type": exc.__class__.__name__, "message": str(exc), "traceback": tb.format_exc()}})
    return 0
