...
class _LoRA_qkv(nn.Module):
    """LoRA wrapper for SAM attention qkv projection."""
...
class LoRA_Adapter_Sam(nn.Module):
    def __init__(self, sam_model: Sam, middle_dim: int, r: int):
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            blk.attn = _Adapter_Attn(atten, w_down_linear_attn, w_up_linear_attn)
            blk.mlp = _Adapter_MLP(mlp, w_down_linear_mlp, w_up_linear_mlp)
    ...
    def save_delta_parameters(self, filename: str) -> None:
        """Save adapter + LoRA + prompt encoder + mask decoder parameters."""
    ...
    def load_delta_parameters(self, filename: str) -> None:
        """Load adapter + LoRA + prompt encoder + mask decoder parameters."""
