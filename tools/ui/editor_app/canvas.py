...
from ui.editor_app.color_utils import label_color
...
class ImageCanvas(QWidget):
    ...
    cursorInfo = Signal(int, int, int)
    brushRadiusChanged = Signal(int)
    maskChanged = Signal()
    roiSelected = Signal(object)
    roiCanceled = Signal()

    def __init__(self, parent: QWidget | None = None, *, render_mode: str = "overlay", editable: bool = True) -> None:
        ...
        self._saved_roi_boxes: list[tuple[QRectF, str, bool]] = []
    ...
    def set_roi_boxes(self, boxes: list[tuple[QRectF, str, bool]]) -> None:
        self._saved_roi_boxes = list(boxes)
        self.update()
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        if self._highlight_boxes and self._render_mode in ("overlay", "image", "mask"):
            self._draw_highlight_boxes(painter, target)
        if self._saved_roi_boxes and self._render_mode in ("overlay", "image", "mask"):
            self._draw_saved_roi_boxes(painter, target)

    def _draw_highlight_boxes(self, painter: QPainter, target: QRectF) -> None:
        if not self._highlight_boxes:
            return
        has_base = (not self._image.isNull() and self._image.width() > 0) or (not self._mask.isNull() and self._mask.width() > 0)
        if not has_base:
            return
    ...
    def _draw_saved_roi_boxes(self, painter: QPainter, target: QRectF) -> None:
        for box, label, is_selected in self._saved_roi_boxes:
            color = QColor(255, 196, 0) if is_selected else QColor(0, 200, 255)
            tl = self._image_to_widget(box.topLeft(), target)
            br = self._image_to_widget(box.bottomRight(), target)
            rect = QRectF(tl, br).normalized()
            pen = QPen(color, 2 if is_selected else 1.5, Qt.PenStyle.DashLine)
            painter.setPen(pen)
            painter.setBrush(Qt.NoBrush)
            painter.drawRect(rect)
            if label:
                ...
                fm = painter.fontMetrics()
                text_rect = fm.boundingRect(label)
                bg_rect = QRectF(rect.left(), rect.top() - text_rect.height() - 4, text_rect.width() + 8, text_rect.height() + 4)
                if bg_rect.top() < target.top():
                    bg_rect = QRectF(rect.left(), rect.top(), text_rect.width() + 8, text_rect.height() + 4)
    ...
    def _target_rect(self) -> QRectF:
        ...
        if not self._image.isNull():
            base_w, base_h = self._image.width(), self._image.height()
        elif not self._mask.isNull():
            base_w, base_h = self._mask.width(), self._mask.height()
        else:
            return QRectF()
        if base_w <= 0 or base_h <= 0 or width <= 0 or height <= 0:
            return QRectF()
        fit_scale = min(width / base_w, height / base_h)
        scale = fit_scale * self._zoom
        target_w, target_h = base_w * scale, base_h * scale
        left = (width - target_w) / 2 + self._pan.x()
        top = (height - target_h) / 2 + self._pan.y()
        return QRectF(left, top, target_w, target_h)
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if self._render_mode == "overlay" and self._roi_mode and event.button() == Qt.LeftButton:
            ...
            if (x1 - x0) < 2 or (y1 - y0) < 2:
                ...
                event.accept()
                return
            self._finish_roi_selection((x0, y0, x1 + 1, y1 + 1))
            event.accept()
            return
        if self._editable and self._render_mode == "overlay" and (not self._roi_mode) and event.button() == Qt.LeftButton:
            self._is_drawing = False
    ...
    def wheelEvent(self, event: QWheelEvent) -> None:
        ...
        if mods & Qt.ControlModifier:
            ...
            old_zoom = self._zoom
            self._zoom = max(0.1, min(20.0, self._zoom * (1.1**step_y)))
    ...
    def _emit_cursor_info(self, widget_pos: QPointF) -> None:
        ...
        if img_pt is None:
            return
        self.cursorInfo.emit(img_pt.x(), img_pt.y(), self._mask_value01(img_pt.x(), img_pt.y()))
    ...
    def _apply_brush(self, widget_pos: QPointF, modifiers: Qt.KeyboardModifiers) -> None:
        ...
        painter.setPen(pen)
        painter.setBrush(Qt.NoBrush)
        painter.drawLine(self._last_img_pt if self._last_img_pt is not None else img_pt, img_pt)
        painter.end()
    ...
    def _rebuild_overlay_cache(self) -> None:
        ...
        color_table = []
        for i in range(256):
            alpha = (i * self._overlay_opacity) // 255
            color_table.append(QColor(255, 0, 0, alpha).rgba())
