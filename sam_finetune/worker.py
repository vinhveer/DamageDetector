...
from sam_finetune.engine import SamFinetuneParams, SamFinetuneRunner


_runner = SamFinetuneRunner()


def _params_from(obj: dict[str, Any]) -> SamFinetuneParams:
    data = dict(obj or {})
    roi = data.get("roi_box")
    if isinstance(roi, list) and len(roi) == 4:
        data["roi_box"] = tuple(int(x) for x in roi)
    allowed = {field.name for field in fields(SamFinetuneParams)}
    data = {key: value for key, value in data.items() if key in allowed}
    return SamFinetuneParams(**data)
...
def _dispatch(proto: WorkerProtocol, call_id: int, method: str, params: dict[str, Any]) -> None:
    ...
    if normalized == "predict_batch":
        def _job():
            ...
            results = []
            log(f"Batch SAM Finetune: {total} images")
